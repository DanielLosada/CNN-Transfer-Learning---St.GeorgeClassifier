...
def train_model(config, train_loader, val_loader):
    ...
    for epoch in range(config["epochs"]):
        ...
        val_loss, val_acc, counter, best_val_loss_after_eval = validate_single_epoch(my_model, criterion, val_loader, (epoch, config["epochs"]), counter, best_val_loss)
    ...
    plot.show()
    save_model(my_model, "my_model_smallnet.pth")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog='St.GeorgeCkassufier Baseline',
    ...
    parser.add_argument("--calcmeanstd",action="store_true", help="Calculate the mean and the std of the dataset.")
